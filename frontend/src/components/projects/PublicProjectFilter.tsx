    ...
    {Object.entries(filterOptions).map(([key, options]) => (
      <div key={key} className="flex items-center gap-3">
        ...
        <select
          ...
          className="font-display text-[0.7rem] font-semibold uppercase tracking-[0.1em] text-copy bg-panel border border-rim px-4 py-2 cursor-pointer outline-none transition-[border-color] duration-300 appearance-none hover:border-accent focus:border-accent"
        >
          {options.map(option => (
