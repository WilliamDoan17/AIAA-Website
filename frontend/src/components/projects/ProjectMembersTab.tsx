...
type AddForm = { memberId: string; role: ProjectMemberRole; title: string }
type AddFieldErrors = Partial<Record<'memberId' | 'title', string>>

const AddMemberModal = ({ projectId, onClose }: AddMemberModalProps) => {
  const [form, setForm] = useState<AddForm>({ memberId: '', role: 'contributor', title: '' })
  ...
  const setField = (key: keyof AddForm, value: string) => {
    ...
    if (fieldErrors[key as keyof AddFieldErrors]) setFieldErrors(e => ({ ...e, [key]: undefined }))
  }
  ...
  const handleSubmit = () => {
    ...
    add(
      { project_id: projectId, member_id: form.memberId, role: form.role, title: form.title.trim() },
      { onSuccess: onClose }
    )
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-void/80 backdrop-blur-sm">
      <div className="bg-panel border border-rim rounded-lg w-full max-w-sm mx-4 p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-body text-xs font-medium text-muted uppercase tracking-widest">Member ID</label>
          <input
            type="text"
            placeholder="Paste member UUID"
            value={form.memberId}
            onChange={e => setField('memberId', e.target.value)}
            className={`bg-surface border rounded px-4 py-2.5 text-sm font-body text-copy placeholder-muted focus:outline-none transition-colors duration-200 ${fieldErrors.memberId ? 'border-red-400' : 'border-rim focus:border-accent'}`}
          />
          {fieldErrors.memberId && <p className="text-red-400 text-xs font-body">{fieldErrors.memberId}</p>}
        </div>
...
const ProjectMembersTab = ({ projectId, canManage }: ProjectMembersTabProps) => {
  ...
  return (
    <>
      {canManage && adding && (
        <AddMemberModal projectId={projectId} onClose={() => setAdding(false)} />
      )}
      {canManage && editing && (
